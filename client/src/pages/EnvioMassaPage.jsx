...
import { api, apiEnvio } from '../utils/api'
import { Card, CardHeader, Spinner, ErrorMsg, Empty, Input, Select, Btn, Badge, PageHeader, StatusBadge } from '../components/UI'
import { Download, Send, Filter, Search, CheckSquare, Square, X, Info, AlertTriangle } from 'lucide-react'
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px,1fr))', gap: 10 }}>
            <div>
              ...
              <Select value={filters.olt} onChange={e => setFilters(f => ({ ...f, olt: e.target.value, slot: '', pon: '' }))} style={{ width: '100%' }}>
                ...
                {(stats?.olts || []).map(o => <option key={o} value={o}>{o}</option>)}
              </Select>
            </div>
            <div>
          ...
          <CardHeader
            ...
            subtitle={
              <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                {comContato > 0 && <Badge color="green" size="sm">{comContato} com WhatsApp</Badge>}
          ...
          <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', background: 'var(--bg-secondary)', borderRadius: '0 0 10px 10px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                O CSV exportado usa o formato <code style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-blue-text)' }}>name, whatsapp</code>.
                Numero no formato <code style={{ fontFamily: 'var(--font-mono)' }}>5516991234567</code>.
              </span>
            </div>
          </div>
        </Card>
